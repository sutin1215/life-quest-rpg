...
> Complete assignments. Defeat bosses. Level up — in the game and in life.
...
Every element of the experience is **uniquely generated by Google Gemini AI** based on who you are — your background, goals, and strengths. No two players have the same character, story, or quest list.

### SDG Alignment

**Primary: SDG 3.4** — Promote mental health and well-being through prevention and treatment.  
...
| Feature | Description |
|---|---|
| 🧙 **AI Character Creation** | Gemini generates your unique RPG class, origin story, and stat allocation from your personal bio |
| 📜 **AI Quest Generation** | Type any real-world task in plain English → Gemini converts it into a lore-flavored RPG quest |
| 🗺️ **World Map & Zones** | 4 unlockable zones (Slime Forest → Goblin Cave → Haunted Keep → Dragon Peak) gated by player level |
| ⚔️ **Boss Battles** | Turn-based combat with Attack / Heal / Ultimate actions; AI narrates every moment |
| 💬 **Dialogue-Gated Battles** | Boss dialogue must fully animate before player can act — genuine RPG conversation pacing |
| 🏰 **Guildmaster Guidance** | AI generates 3 personalized daily quests based on your full player profile |
| 🛒 **Goblin Market Shop** | Spend earned gold to permanently upgrade STR, INT, or DEX stats |
| 🎭 **Pixel Boss Art** | Custom-painted pixel art for each boss (Giant Slime, Goblin King, Skeleton Lord, Red Dragon) |
| 🌟 **Floating Rewards** | Animated +XP and +Gold popups on quest completion |
| 📊 **Stat Progression** | Three stats (STR / INT / DEX) that grow with quest completions and shop purchases |
| 🎆 **Level-Up Celebrations** | Full-screen confetti + dialog on level up |
| 🎨 **Zone Backgrounds** | Hand-painted `CustomPainter` backgrounds unique to each battle zone |
...
| **Firebase Auth** | Anonymous user authentication | Zero-friction onboarding — no account required; unique UID persists across sessions |
...
│                                                                 │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────────┐   │
│  │  Screens │  │ Widgets  │  │ Services │  │    Theme     │   │
│  │          │  │          │  │          │  │              │   │
│  │ splash   │  │dialogue  │  │AiService │  │  RpgTheme    │   │
│  │ home     │  │  box     │  │          │  │  Colors      │   │
│  │ battle   │  │boss      │  │Database  │  │  Styles      │   │
│  │ world map│  │painter   │  │Service   │  │  Decorations │   │
│  │ shop     │  │floating  │  │          │  │              │   │
│  │ profile  │  │reward    │  │          │  │              │   │
│  │ char     │  │glowing   │  │          │  │              │   │
│  │ creation │  │stat badge│  │          │  │              │   │
...
```
users/                          ← collection
  {uid}/                        ← document (one per anonymous user)
...
Firebase Anonymous Auth
    │
    ├─── New User ──► CharacterCreationScreen
    │                       │
    │                 [Bio + Main Quest input]
    │                       │
    │               Gemini generateCharacter()
    │                       │
    │               Firestore: initializeUser()
    │                       │
    │                 ProfileScreen
    │                       │
    │               Gemini generateStarterQuests()
    │                       │
    │               Firestore: addQuests()
    │                       │
    └─── Existing User ─────┤
                      ...
                      ├── QuestBoard (Quests tab)
                      │     ├── [+] Add Quest → Gemini generateQuest()
                      │     ├── Guildmaster → Gemini generateGuildmasterQuests()
                      │     └── Complete Quest → Firestore transaction
                      │           └── XP/Gold/Stat update → Level-up check
                      ├── WorldMapScreen (Map tab)
                      │     └── Zone tap → BattleScreen
                      │           ├── Gemini: opening boss taunt
                      │           ├── Turn loop (Attack/Heal/Ultimate)
                      │           │     ├── Boss HP < 50% → mid-battle taunt
                      │           │     └── Dialogue gate (CONTINUE ▶)
                      │           └── Victory/Defeat → Gemini narration
                      ├── ShopScreen (Shop tab)
                      │     └── Buy item → Firestore: spend gold, gain stat
                      └── ProfileScreen (Hero tab)
                            └── View class, stats, story, XP progress
```
...
// Gemini 2.5 Flash — accuracy & JSON reliability
...
// Gemini 2.5 Flash Lite — speed & low latency
...
A key UX innovation — boss dialogue must complete its typewriter animation before the player can act, creating genuine RPG conversation pacing:
...
// Player taps CONTINUE → action buttons unlock
...
Quest completion uses a Firestore transaction to atomically update XP, gold, stat, and check for level-up — preventing race conditions if the user taps rapidly:
...
All boss art and battle backgrounds are drawn entirely in Flutter using `CustomPainter` — no external image assets. This keeps the APK size minimal while giving each zone and boss a unique visual identity. For example, the Giant Slime boss:

```dart
// In boss_painter.dart — pure geometry, no images
void _drawSlime(Canvas canvas, Size size, Color color, double sz) {
  final body = Paint()..color = color;
  // Layered ellipses, eyes, highlights, shadows — all code
  canvas.drawOval(Rect.fromCenter(...), body);
  // ... shimmer highlights, blinking eyes, drip effects
}
```

### 6. Guildmaster — Profile-Aware Quest Generation
...
A Level 3 hero with low DEX receives different guidance than a Level 8 hero with balanced stats — the AI reasons about what is most impactful for this specific player profile.
...
|---|---|
| Character creation | Becomes a preset class picker — no personal connection |
...
> ⚠️ **Critical:** The `.env` file is already in `.gitignore`. Never commit it to version control — your key will be automatically blocked by Google if exposed on GitHub.
...
├── ios/
│   └── Runner/
│       └── GoogleService-Info.plist ← Add this file (iOS only)
└── ...
```
...
3. Check Firestore — if no user document exists, navigate to **Character Creation**
...
2. Click **Add project** → name it (e.g., `life-quest-2026`)
...
│
├── models/
│   └── quest_model.dart         # Quest data class
│
├── screens/
│   ├── splash_screen.dart       # Animated title screen (3.2s, 60 stars)
│   ├── character_creation_screen.dart  # Bio + Main Quest input → Gemini
│   ├── profile_screen.dart      # Hero stats, story, class display
│   ├── home_screen.dart         # Bottom nav + QuestBoard + Guildmaster
│   ├── world_map_screen.dart    # Zone selection with animated markers
│   ├── battle_screen.dart       # Turn-based combat with dialogue gate
│   └── shop_screen.dart         # Goblin Market — spend gold for stats
│
├── services/
│   ├── ai_service.dart          # All Gemini API calls (2 models, retry logic)
│   └── database_service.dart    # All Firestore operations (streams + writes)
│
├── theme/
│   └── rpg_theme.dart           # Central color palette, text styles, decorations
│
└── widgets/
    ├── rpg_dialogue_box.dart    # Typewriter animation widget with onComplete callback
    ├── boss_painter.dart        # CustomPainter pixel art for all 4 bosses
    ├── battle_backgrounds.dart  # CustomPainter zone backgrounds (4 themes)
    ├── floating_reward.dart     # Animated +XP / +Gold popup overlay
    └── glowing_stat_badge.dart  # Pulsing STR/INT/DEX stat display widgets
```
...
**What happened:** Flutter's linter flagged `ScaffoldMessenger.of(context)` calls that appeared after `await` statements — a legitimate warning since the widget may have unmounted during the async gap.

**Resolution:** Captured the messenger reference *before* the `await`:

```dart
// ✅ Correct — capture before async gap
...
### Phase 1: 0–6 Months — Stabilise & Grow
...
### Phase 2: 6–12 Months — Community Layer
- [ ] **Guild System** — Form study/accountability groups; cooperative boss raids where each member's quest completions deal collective damage
- [ ] **Weekly Challenges** — Gemini-generated community quests with leaderboards
- [ ] University pilot programme — institution-customized zones (e.g., "The Finals Dungeon" during exam season)
- [ ] Push notifications for quest reminders and level-up celebrations

### Phase 3: 12+ Months — Platform Expansion
- [ ] **B2B Corporate Wellness** — Custom-branded deployments for HR teams with employee wellness quest templates
- [ ] **Wearable Integration** — Auto-generate STR quests from Google Fit / Apple Health data
- [ ] **SEA Localization** — Bahasa Melayu, Bahasa Indonesia, Thai, Vietnamese
- [ ] **Multiplayer Guilds** — Real-time guild chat, shared quest boards, and guild wars
